...
const sentimentBlock = `function QuickStatsSentimentBlock({ sentiment }) {
  if (sentiment.mode === 'segments') {
    return (
      <motion.div className="flex flex-col gap-[10.287px] pb-[2.546px] pt-[2.559px]">
  ...
  return (
    <div className="flex flex-col gap-[10px]">
      ...
      <div className="flex h-[10px] w-full overflow-hidden rounded-[5px]">
        <motion.div className="h-full w-full rounded-[5px]" style={{ backgroundColor: accentColor }} />
      </div>
...
const quickStatsCard = `function QuickStatsCard({ form }) {
...
// Fix typos in quickStatsCard - I used motion.div wrongly. Let me fix the template
const quickStatsCardFixed = quickStatsCard
  .replace(/<\/motion\.motion\.motion\.div>/g, '</div>')
  .replace(/<motion\.div className="h-full w-full/g, '<div className="h-full w-full')
  .replace(
    `          <p className="shrink-0 text-[40px] font-medium leading-[41.148px] text-[#15140e]">42%</p>
        </motion.div>
      </motion.div>`,
...
const recommendedCompact = `function RecommendedActionCompactCell({ action }) {
  return (
...
// Too many typos in script - write clean versions to separate files and splice
