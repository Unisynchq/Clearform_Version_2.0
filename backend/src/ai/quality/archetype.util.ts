...
const REVIEW_KEYWORDS =
  /\b(review|360|feedback|performance review|internal)\b/i;
...
function templateIdFromSnapshot(snapshot: unknown): string {
  ...
  return String(
    (snapshot as Record<string, unknown>).templateId ?? '',
  ).toLowerCase();
}
...
export function detectArchetype(
  ...
  const hasRating = screens.some((s) => {
    ...
    return (
      String(s.label ?? '')
        .toLowerCase()
        .includes('rating') || cfg.ratingQuestion != null
    );
  });
  ...
  if ((NPS_KEYWORDS.test(haystack) || NPS_KEYWORDS.test(labels)) && hasRating) {
    return 'customer-nps';
  }
  if (
